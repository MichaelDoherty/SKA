...
{
	if (net_server != NULL) delete net_server;
	net_server = NULL;
	results_file.close();
	exit(_exit_code);
}
...
//static char sendbuf[buffer_len];
static int sendbuflen = buffer_len;
static double net_send_frame = 1.0;
static double net_send_countdown = 0;
static int net_x = 0;
static int net_y = 50;
...
struct BoneData {
	short id;
	Vector3D p1, p2;
	BoneData(short _id, Vector3D _p1, Vector3D _p2)
		: id(_id), p1(_p1), p2(_p2) {}
};
...
void updateAnimation()
{
	...
	processPTData();
	net_send_countdown -= elapsed_time;
	net_x = (net_x++) % 400;
	net_y = (net_y++) % 200;
	if (net_send_countdown < 0.0) {
		sendNetDataStream();
		net_send_countdown = net_send_frame;
	}
}
